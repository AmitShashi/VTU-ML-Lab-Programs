class Graph:
    ...
    def computeMinimumCostChildNodes(self, v):  # Computes the Minimum Cost of child
        ...
        flag = True
        for nodeInfoTupleList in self.getNeighbors(v):  # iterate over all the set child node/s
            cost = 0
            nodeList = []
    ...
    def aoStar(self, v, backTracking):  # AO* algorithm for a start node and BackTracking status flag
        ...
        if self.getStatus(v) >= 0:  # if status node v >= 0, compute Minimum Cost nodes of v
            ...
            if solved == True:  # if the Minimum Cost nodes of v are solved, set the current node status as solved(-1)
                self.setStatus(v, -1)
                self.solutionGraph[
                    v] = childNodeList  # update the solution graph wit the solved nodes which may be a part of solution
            ...
            if backTracking == False:  # check the current call is not for backtracking
                for childNode in childNodeList:  # for each Minimum Cost child node
                    self.setStatus(childNode, 0)  # set the status of child node to (needs exploration)
